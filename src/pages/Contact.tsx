...
export const Contact: React.FC = () => {
  const { ticketTokenId, isVictoryEdition, setVictoryEdition, setCheckedIn } = useWeb3();
  ...
  // Sync state if feed changes
  useEffect(() => {
    if (feed) {
      setCurrentScore(feed.score);
      setCurrentMinute(feed.minute);
    }
  }, [feed]);
    ...
    <div style={{ flex: 1, paddingBottom: '5rem' }}>
      <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '3rem 1.5rem' }}>
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2.2rem, 5vw, 3rem)', marginBottom: '0.75rem' }}>
            Admin Simulation Command Center
          </h1>
          <p style={{ color: 'var(--color-text-secondary)', fontFamily: 'var(--font-family-body)', fontSize: '0.95rem', lineHeight: 1.65 }}>
            Manage match states, deploy real-time events, and monitor AI Fan Agent telemetry logs.
          </p>
        </div>
        ...
        <div className="dashboard-grid" style={{ padding: 0, gap: '2rem' }}>
          ...
          {/* LEFT COLUMN: Global Match Controls & Feed */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            
            {/* Event Customizer / Deployment Panel */}
            <EventControlPanel />

            {/* Quick Demo Checks Card */}
            <div className="glass-panel" style={{ padding: '2rem' }}>
              <h3 style={{ fontSize: '1.05rem', marginBottom: '0.75rem', color: 'var(--color-text-primary)' }}>
                🛠 Direct User-State Simulators
              </h3>
              <p style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)', fontFamily: 'var(--font-family-body)', marginBottom: '1.25rem' }}>
                Simulate on-chain user actions directly without scanning or waiting for feed triggers.
              </p>
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                <button
                  onClick={() => setVictoryEdition()}
                  style={{
                    padding: '0.6rem 1.25rem', borderRadius: 'var(--radius-pill)', cursor: 'pointer',
                    background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.3)',
                    color: '#D97706', fontWeight: 700, fontSize: '0.82rem',
                    fontFamily: 'var(--font-family-body)', transition: 'all var(--transition-fast)',
                  }}
                >
                  🏆 Force Victory Upgrade (My NFT)
                </button>
                <button
            ...
            <div className="glass-panel" style={{ padding: '2rem' }}>
              <h3 style={{ fontSize: '1.05rem', marginBottom: '1.25rem', color: 'var(--color-text-primary)' }}>
                📖 REST API Endpoints
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[
                  { method: 'POST', path: '/api/admin/simulate-trigger', desc: 'Trigger match event (NONE | GOAL | MATCH_END_WIN)' },
                  { method: 'POST', path: '/api/validator/scan', desc: 'Simulate turnstile QR code scanner authentication' },
          ...
          {/* RIGHT COLUMN: AI Fan Agent Telemetry & Logs */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            
            {/* AI Fan Agent simulation console */}
            <div className="glass-panel" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '1.25rem', color: 'var(--color-text-primary)' }}>⚽ AI Fan Agent Simulator</h3>
                <span style={{
                  padding: '0.35rem 0.9rem', borderRadius: 'var(--radius-pill)',
            ...
            {/* AI Fan Agent Telemetry Terminal Box */}
            <div className="glass-panel" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '1.2rem', color: 'var(--color-text-primary)' }}>🤖 AI Fan Manager Telemetry (fanManager.ts)</h3>
              ...
              <p style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)', fontFamily: 'var(--font-family-body)', margin: 0 }}>
                Real-time terminal output from the AI Fan Engagement Agent monitoring match events and executing updates.
              </p>

              <div style={{
                height: '300px', background: '#0A0A0F', borderRadius: '14px',
              ...
              <div style={{
                ...
                <span style={{ fontSize: '0.78rem', color: 'var(--color-success)', fontWeight: 700 }}>Injective EVM Bridge ✓</span>
              </div>
            </div>
